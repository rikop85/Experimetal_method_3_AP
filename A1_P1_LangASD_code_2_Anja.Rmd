...
```{r}
demo_train = read.csv("demo_test.csv")
LU_train = read.csv("LU_tesr.csv")
token_train = read.csv ("token_test.csv")
```
...
demo_train_sub = select(demo_train, subj_id, visit, Ethnicity, Diagnosis, Gender, Age, ADOS, MullenRaw, ExpressiveLangRaw)

lu_train_sub = select(LU_train, subj_id, visit, MOT_MLU, MOT_LUstd, CHI_MLU, CHI_LUstd)

token_train_sub = select(token_train, subj_id, visit, types_MOT, types_CHI, tokens_MOT, tokens_CHI)
...
write.csv(data02, file = "final_A1.csv")
...
#gruppere dit datasæt efter Child.ID, hvorefter du kan bruge funktionen summarise sammen med n(), for at ende ud med et data sæt som indeholder en række pr. barn med en tilhørende værdi (svarende til antal besøg).Herefter kan du bruge filter på dit nye datasæt, til at filtrere alle de børn fra som ikke har en værdi af 6 

data03 = data02 %>%
  group_by(as.factor(subj_id)) %>%
  summarise(visit = n())

#I cannot get this part to work properly
...
SEL_01 = select(data02, contains(data02$Diagnosis =="ASD"), CHI_MLU, tokens_CHI, subj_id)

#Making a dataframe with all variables I need
SEL_001 = select(data02, Diagnosis, CHI_MLU, tokens_CHI, subj_id)
...
#2 - isn't it the same as the one above? At least my results are, so something is off. 
